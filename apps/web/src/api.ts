...
export type ImportResult = { filename: string; status: string; created_count: number; updated_count: number; skipped_count: number; errors: Array<{ row: number; message: string }> }
...
export type PatientQuery = { nationalId?: string; firstName?: string; lastName?: string; district?: string; subdistrict?: string; versions?: string[]; status?: string }
...
export const api = {
  ...
  patients: (token: string, query: PatientQuery = {}) => {
    ...
    return request<{ items: Patient[]; total: number }>(`/patients?${params}`, token)
  },
