...
export type PatientFilters = { districts: string[]; subdistricts: string[]; genders: string[] }
export type PatientQuery = { nationalId?: string; firstName?: string; lastName?: string; district?: string; subdistrict?: string; gender?: string; versions?: string[]; status?: string; page?: number; pageSize?: number }

function queryParams(query: PatientQuery, includePaging = false) {
  ...
  if (includePaging) { params.set('page', String(query.page ?? 1)); params.set('page_size', String(query.pageSize ?? 20)) }
  return params
}
...
export const api = {
  ...
  summary: (token: string, query: PatientQuery = {}) => request<Summary>(`/dashboard/summary?${queryParams(query)}`, token),
  patients: (token: string, query: PatientQuery = {}) => {
    const params = queryParams(query, true)
  ...
  unions: (token: string, query: PatientQuery = {}) => request<Record<string, number>>(`/dashboard/unions?${queryParams(query)}`, token),
