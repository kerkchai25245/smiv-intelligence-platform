...
export function Dashboard({ token }: { token: string }) {
  ...
  return <Stack spacing={3}>
    ...
    {importResult && <Alert severity={importResult.skipped_count ? 'warning' : 'success'} onClose={() => setImportResult(null)}>นำเข้าเสร็จแล้ว — เพิ่ม {importResult.created_count.toLocaleString()} | อัปเดต {importResult.updated_count.toLocaleString()} | <strong>นำเข้าแต่รอตรวจแก้ {importResult.skipped_count.toLocaleString()}</strong>{importResult.skipped_count > 0 && ' (ข้อมูลไม่สูญหาย)'}</Alert>}
    ...
    {issueTotal > 0 && <Card sx={{ border: '2px solid', borderColor: 'error.main', bgcolor: 'rgba(211,47,47,.06)' }}><CardContent>
